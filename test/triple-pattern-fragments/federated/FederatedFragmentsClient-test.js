  ...
  describe('A FederatedFragmentsClient with a start fragments', function () {
    ...
    describe('when asked for ?s ?o dbpedia:York', function () {
      ...
      describe('and receiving a Turtle response', function () {
        var fragment = AsyncIterator.wrap(fs.createReadStream(__dirname + '/../../data/fragments/$-birthplace-york.ttl'));
        var fragmentDbplive = AsyncIterator.wrap(fs.createReadStream(__dirname + '/../../data/fragments/$-birthplace-york-dbplive.ttl'));
        ...
        it('should stream the triples in the fragment', function (done) {
          result.should.be.an.asyncIteratorWithLength(38, done);
        });
