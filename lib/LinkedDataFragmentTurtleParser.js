...
LinkedDataFragmentTurtleParser.prototype = {
  ...
  parse: function (document, documentUrl) {
    ...
    new N3Parser({ documentURI: documentUrl }).parse(document, function (error, triple) {
      ...
      if (!count && triple.predicate === countPredicate &&
          (documentUrl === triple.subject || decodedURIEquals(documentUrl, triple.subject)))
...
// Check whether two URIs are equal after decoding, to make up for encoding differences
