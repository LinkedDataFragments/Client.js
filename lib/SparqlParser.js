...
// Query templates
var prefixesTemplate = /^\s*(PREFIX [\-a-z0-9]*:\s*<[^>]*>\s+)*/i;
...
SparqlParser.prototype = {
  ...
  parse: function (query) {
    var match, prefixes = query.match(prefixesTemplate)[0];
    query = query.substr(prefixes.length);

    if (match = query.match(selectTemplate))
      return this._parseSelect(match[1].split(/\s+/), prefixes + match[2], match[3]);
