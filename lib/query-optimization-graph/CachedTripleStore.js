 ...
 */
/* Triple store interface that caches previous (intermediate) query results. */
...
// Removes all cache entries references the given node.
...
// Estimates the optimal path for the given nodes based on the previous results.
...
// Returns bindings from oldEntry that were not yet used to create bindings in newEntry.
...
// Find the largest up to date cache entry matching (a subset of) the given nodes
...
// Helper function for maxMatchingResult. Checks if the given entry is up to date for the given nodes.
...
// Main function of the class. Finds all bindings matching the given nodes.
// If v is provided, returns the unique values of that variable in the found bindings.
N3StoreInterface.prototype.matchBindings = function (nodes, callback, v) {
  ...
  if (nodes.length > 0) {
    while (nodes.length > 0) {
      ...
      var uniques = null, vv;
      if (cacheEntry) {
        // found matching entry: there are results we can reuse
      ...
      // only happens if we found no cacheEntry
      if (uniques)
        for (vv in uniques)
...
// Finds unique binding values in the set of bindings for the variable v.
...
// Merges leftBindings with the given triples that conform to the pattern.
// Offset is used to make sure the indices of the created binding objects are correct.
// This is done by creating a lookup tree of the smallest set of bindings with a branch level for every different variable.
// The other set of bindings is than matched with this tree.
...
// Create a tree for a set of bindings.
// The number of levels is equal to the number of different variables in the bindings.
// The actual bindings are stored in arrays in the leaves.
...
// Converts triples to binding objects.
// We store the binding objects in the triples for maximal reuse.
