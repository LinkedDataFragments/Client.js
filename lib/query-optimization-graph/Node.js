...
Node.prototype.updateDependency = function () {
  //for (var v in this.bindStreams)
  //  this.bindStreams[v].dependencies = this.generateStreamDependencies(v);
  this.activeStream.dependencies = this.generateStreamDependencies(this.activeStream.bindVar);
  //this.logger.info('dependencies updated: ' + _.map(this.dependencies, function (node) { return rdf.toQuickString(node.pattern); }));
};

// TODO: if we use this for all streams we really need to make sure the suppliers are in here
...
//  }
  ...
  if (bindStream && (bindStream === this.activeStream)) { // bindStream.dependencies.length > 0
    this.DEBUGclusters[v].matchSuppliers(bindStream.dependencies, function (specificBindings, ownEstimate, ownMatchRates, ownEstimates) {
      ...
      // take wrong estimations into account
...
Node.prototype.cost = function () {
  ...
  return this.activeStream.costRemaining * this._costModifier(this.activeStream);
};
...
Node.prototype.read = function (callback) {
  ...
  this.activeStream.read(function (buffer, remove) {
    ...
    //if (self.activeStream === self.fullStream)
    //  _.each(self.equivalents, function (node) { node.triples = self.triples; });
    setImmediate(callback(buffer));
  }, function (preReadTime, readTime, postReadTime, addPre, addRead, addPost) {
    if (!self.DEBUGclusters[ClusteringUtil.getVariables(self.pattern)[0]])
      return; // happens if node has no variables // TODO: complete fix
    var DEBUGtimer = self.DEBUGclusters[ClusteringUtil.getVariables(self.pattern)[0]].DEBUGcontroller.DEBUGtimer;
