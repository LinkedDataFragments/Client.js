...
N3StoreInterface.prototype.matchBindings = function (nodes, callback, v) {
  ...
  if (nodes.length > 0) {
    ...
    for (var i = bestCache.nodes.length; i < orderedNodes.length; ++i) {
      ...
      start = new Date();
      if (cacheEntry) {
        var incompleteStrings = _.filter(Object.keys(prevEntry.counts), function (str) { return cacheEntry.counts[str] < prevEntry.counts[str]; });
        var filtered = [];
        if (incompleteStrings.length > 0) {
          for (var j = 0; j < prevEntry.bindings.length; ++j) {
            var binding = prevEntry.bindings[j];
            for (var k = 0; k < incompleteStrings.length; ++k) {
              var str = incompleteStrings[k];
        ...
        var newBindings = this.extendBindings(bindings, node.pattern, newTriples, cacheEntry.counts[ClusteringUtil.tripleID(node.pattern)], last ? v : null);
        DEBUGTIMER.newTriples += new Date() - start;

        start = new Date();
        if (last && v) {
          if (cacheEntry.bindings)
            uniques = self.union(_.map(cacheEntry.bindings, function (bindObj) { return bindObj.binding[v]; }), oldBindings[1], newBindings[1]);
          else
            uniques = self.union(oldBindings[1], newBindings[1]);
        }
...
//      this.logger.info(_.map(Object.keys(DEBUGSTUFF), function (key) { return key + ': ' + DEBUGSTUFF[key]; }).join(', '));
      } else {
        start = new Date();
...
//    var uniq = _.uniq(bindings, function (binding) { return _.map(_.sortBy(Object.keys(binding.binding)), function (key) { return key+''+binding.binding[key]; }).join(''); } );
//    bindings = uniq;
      //node.newTriples = [];
      if (cacheEntry)
      ...
      // TODO: can keep them more often if we calculate more unique values, need to calculate tradeoffs
      cacheEntry.uniques = {};
      if (last && v)
        cacheEntry.uniques[v] = uniques;
...
var DEBUGTIMER = {pre:0, find:0, newTriples:0, updatedTriples:0, filter:0, basic:0, rest:0, extend:0, extend_triples:0, extend_tree:0, extend_merge:0, bestCache:0, uniq:0, uniq_final:0, block:0, total:0};
...
N3StoreInterface.prototype.union = function (/*args*/) {
  var cache = {};
  var k;
...
N3StoreInterface.prototype.extendBindings = function (leftBindings, pattern, triples, offset, v) {
  ...
  if (!leftBindings) {
    var uniqs = [];
    if (v)
      uniqs = this.uniq(rightBindings, v);
  ...
  for(i = 0; i < maxBindings.length; i++) {
    ...
    if (valid) {
      ...
      for (j = 0; j < branch.length; j++) {
        ...
        //else
        var merge = this.mergeBindings(binding, branch[j], v);
        joined.push(merge[0]);
        if (v)
  ...
  return [joined, Object.keys(vResults)];
};
...
N3StoreInterface.prototype.mergeBindings = function (left, right, v) {
  ...
  for (variable in left.binding) {
    binding = left.binding[variable];
    if (variable === v)
      val = binding;
    merged.binding[variable] = binding;
  }

  for (variable in right.binding) {
    binding = right.binding[variable];
    if (variable === v)
      val = binding;
    merged.binding[variable] = binding;
  }
