  ...
  for (var i = 0; i < orderedNodes.length; ++i) {
    ...
    if (cacheEntry && this.cacheMatchesNodes(cacheEntry, orderedNodes.slice(0, i+1)) && (!finalEntry || this.cacheMatchesNodes(finalEntry, orderedNodes.slice(0, i+1)))) {
      ...
      if (cacheEntry) {
        ...
        start = new Date();
        var updatedNewBindings = this.extendBindings2(newBindings, node.pattern, this.patternFromStore(node.pattern));
...
N3StoreInterface.prototype.extendBindings2 = function (leftBindings, pattern, triples) {
  ...
  // merging
  var tree = {}, tmp, binding, variable, val, values, i, j;
  for(i=0; i < leftBindings.length; i++) {
    binding = leftBindings[i];
    tmp = tree;
    for(j=0; j<keys.length; j++) {
      variable = keys[j];
      val = binding[variable];
      if(j == keys.length-1) {
        values = tmp[val] || [];
        values.push(binding);
        tmp[val] = values;
  ...
  for(i=0; i<rightBindings.length; i++) {
    ...
    for(j=0; j<keys.length; j++) {
      ...
      if(tmp[val]) {
        if(j == keys.length-1) {
          for(var k=0; k<tmp[val].length; k++) {
            joined.push(this.mergeBindings2(tmp[val][k],binding));
          }
        } else {
          tmp = tmp[val];
        }
