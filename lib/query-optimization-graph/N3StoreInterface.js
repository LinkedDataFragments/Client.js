  ...
  if (nodes.length > 0) {
    ...
    for (var i = bestCache.nodes.length; i < orderedNodes.length; ++i) {
      ...
      if (cacheEntry) {
        ...
        var filtered = incompleteStrings.length === 0 ? [] : _.filter(prevEntry.bindings, function (binding) {
        ...
        start = new Date();
        if (last && v) {
          // TODO: because of workaround...
          if (cacheEntry.bindings.length === 1 && !cacheEntry.bindings[0].binding)
...
//    bindings = uniq;
  ...
  if (v) {
    ...
    if (cachedBindings === bestCache.bindings && cachedBindings.length > 0 && cachedBindings[0].bindings)
