...
N3StoreInterface.prototype.cacheCost = function (node, prevEntry) {
  ...
  // this looks more complicated than the other version, but gives the same results...
  //return newCount*(1+prevEntry.bindings.length) + (diffs.length > 0 ? Math.max.apply(null, diffs) : 0)*oldCount;
  return newCount;
};
    ...
    for (var i = bestCache.nodes.length; i < orderedNodes.length; ++i) {
      ...
      if (cacheEntry) {
        ...
        if (incompleteStrings.length > 0) {
          for (var j = prevEntry.bindings.length-1; j >= 0; --j) {
            var match = false;
        ...
        //if (last && v) {
          ...
          //  uniques = self.union(_.map(cacheEntry.bindings, function (bindObj) { return bindObj.binding[v]; }), oldBindings[1][v] || [], newBindings[1][v] || []);
          //else
          //  uniques = self.union(oldBindings[1][v] || [], newBindings[1][v] || []);
        //}
