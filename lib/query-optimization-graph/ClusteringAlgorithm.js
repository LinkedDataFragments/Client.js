...
function ClusteringAlgorithm (clustering) {
  ...
  this._logger = new Logger("ClusteringAlgorithm");
}
...
ClusteringAlgorithm.prototype.run = function (callback) {
  ...
  _.each(clustering.nodes, function(node) {
    self._logger.info("Init (1): Count " + rdf.toQuickString(node.pattern));
  ...
  function step(){
    ...
    if (clustering.isFinished()) {
      self._logger.info("TOTAL CALLS: " + cost);
    ...
    var calls = _.filter(
      [
        self._minimalFullDownload(),
        self._minimalSingleVarCount(),
        self._minimalSingleVarDownload(),
        self._minimalFullBindingDownload()
      ],
      _.identity
    );
    var minCall = _.min(
    ...
    cost += minCall.cost;
    self._logger.info("Best (" + minCall.cost + "): " + minCall.description);
    minCall.call(step);
  }
};
...
ClusteringAlgorithm.prototype._minimalFullDownload = function () {
  ...
  return _.min(ClusteringAlgorithm._createCallsFromNodeList(
      ...
      function(node, callback) {
        clustering.downloadNode(node, function() { callback(); });
      },
...
ClusteringAlgorithm.prototype._minimalFullBindingDownload = function () {
  ...
  return _.min(ClusteringAlgorithm._createCallsFromNodeList(
      ...
      function (node) {
        ...
        return _.reduce(ClusteringUtil.getVariables(node.pattern), function (memo, v) {
