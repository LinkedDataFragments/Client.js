...
BindingStream.prototype.feed = function (add, remove) {
  ...
  this._bindings = _.difference(this._bindings, remove);
  var groupedResults = _.groupBy(this.results, function (result) {
    return _.contains(remove, _.first(_.keys(result))) ? "remove" : "keep";
  });
  ...
  this._streams = _.filter(this._streams, function (stream) {
    var shouldRemove = _.contains(remove, stream.bindVal);
