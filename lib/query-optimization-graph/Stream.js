...
function Stream (cost, pattern, loggername)
{
  ...
  this.estimates = {};
  var self = this;
  // TODO: are these actually still used for something?
...
DownloadStream.prototype.read = function (callback) {
  ...
  function addBuffer (buffer) {
    ...
    if (self.tripleCount > self.count)
      self.count = self.tripleCount + 1; // wrong server estimation
...
BindingStream.prototype.isStable = function () {
  ...
  var prevMargin = 0.98/Math.sqrt(_.size(this.results))*prevAvg;
...
BindingStream.prototype.read = function (callback, timingCallback, _recursive) {
  ...
  // TODO: we download the the first page to get counts, should already give the triples downloaded then
  if ((!_recursive || !this.isStable() || _.isEmpty(this._streams)) && !_.isEmpty(this._bindings)) {
...
// TODO: can we assume that the input list will only grow (so that we just need to take a sublist)
BindingStream.prototype.feed = function (bindings) {
  ...
  for (i = 0; i < this.resultVals.length; ++i)
    delete cache[this.resultVals[i]];
