...
BindingStream.prototype.resultsPerBinding = function (results) {
  ...
  var result = _.reduce(_.map(results, function (result) { return _.first(_.values(result)); } ), function (memo, val) { return memo + Math.max(val, 1) ; }, 0) / _.size(results);
...
BindingStream.prototype.updateRemaining = function (remaining) {
  ...
  this.remaining = _.reduce(_.pluck(this._streams, 'remaining'), function (memo, val) { return memo + val; }, 0);
  this.remaining += (remaining + _.size(this._bindings)) * this.resultsPerBinding();

  var diff = this.remaining - this.cost;
  this.cost = this.remaining;
  this.costRemaining = Math.min(this.cost, this.costRemaining + diff);

  this.count = _.reduce(_.map(this.results, function (result) { return _.first(_.values(result)); } ), function (memo, val) { return memo + val; }, 0);
