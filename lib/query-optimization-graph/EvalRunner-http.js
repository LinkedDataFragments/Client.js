...
// mostly copy/paste of ldf-client-http with additional debug code
...
function handleRequest(request, response) {
  ...
  switch (request.method) {
    ...
    case 'GET':
      return writeQueryResult(request, response, requestUrl.query.query || '', requestUrl.query.algorithm || '1');
    default:
...
function writeQueryResult(request, response, query, algorithm) {
  ...
  var hasTriplesResult = /\s*(?:CONSTRUCT|DESCRIBE)/i.test(query),
    ...
    client = spawn(node, clientArgs.concat(['-t', mimeType, '-q', query, '-a', algorithm]), ioSettings);
