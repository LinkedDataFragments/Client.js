...
ClusteringController.prototype.start = function () {
  ...
  var parsedNodes = _.sortBy(_.filter(this.nodes, function (node) { return node.activeStream === node.fullStream; }), function (node) {return node.fullStream.count;});
  ...
  self.logger.info("node order: " + _.map(_.pluck(this.nodes, 'pattern'), rdf.toQuickString));
