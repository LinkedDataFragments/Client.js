...
//    hungryClusters = this.clusters;
  ...
  var delayedCallback = _.after(_.size(this.clusters), function () {
    ...
    if (minNode === Infinity) {
      ...
      //console.log("HTTP calls: " +  self.nodes[0]._options.fragmentsClient._client.DEBUGcalls);
      ...
      //console.log(_.map(_.keys(self.store.DEBUGTIMERTOTAL), function (key) { return key + ': ' + self.store.DEBUGTIMERTOTAL[key]; }).join(', '));
      self.callback([], true);
...
//    });
    ...
    var delayedCallback = _.after(_.size(self.clusters), function () {
      ...
      self.store.matchBindings(self.nodes, function (results) {
        ...
        var newResults = results.slice(_.size(self.results)); // use _.size since self.results is a map
        ...
        if (results.length > self.results.length)
          self.logger.info("DUPLICATE RESULTS!");
