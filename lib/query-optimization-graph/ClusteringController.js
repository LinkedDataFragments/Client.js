...
//    });
  ...
  this.DEBUGtimer = {first:0, preread:0, write_file:0, read:0, http:0, read_pre:0, read_read:0, read_post:0, read_add_pre:0, read_add_read:0,
...
//    hungryClusters = this.clusters;
  ...
  var delayedCallback = _.after(_.size(this.clusters), function () {
    ...
    if (minNode === Infinity) {
      ...
      self.logger.info("HTTP call time: " +  Math.floor(self.nodes[0]._options.fragmentsClient._httpClient.DEBUGtime));
      self.logger.info("HTTP calls: " +  self.nodes[0]._options.fragmentsClient._httpClient.DEBUGcalls);
      self.logger.info("Remaining time: " + Math.floor(self.DEBUGtime - self.store.DEBUGtime - self.nodes[0]._options.fragmentsClient._httpClient.DEBUGtime));
...
ClusteringController.prototype.readNode = function (minNode) {
  ...
  minNode.read(function (add, remove) {
    ...
    self.DEBUGtimer.http = Math.floor(self.nodes[0]._options.fragmentsClient._httpClient.DEBUGtime);
...
//    });
    ...
    var delayedCallback = _.after(_.size(self.clusters), function () {
        ...
        if (_.isFinite(node.activeStream.cost) && node.activeStream.cost > 1.1*node.fullStream.cost) { // 1.1 to prevent unnecessary switching when the difference is small
          _.each([node].concat(node.equivalents), function (node) {
            if (node.activeStream === node.fullStream)
              return;
            node.logger.info("SWITCH STREAM " + v + " -> " + undefined + ", estimate: " + self.clusters[v].estimate + ", oldCost: " + node.activeStream.cost + ", cost: " + node.fullStream.cost);
            node.activeStream = node.fullStream;
      ...
      self.store.matchBindings(self.nodes, function (results) {
        ...
        if (gotResults) {
          if (self.DEBUGtimer.first <= 0)
            self.DEBUGtimer.first = new Date() - self.DEBUGstart;
          self.DEBUGtime = new Date() - self.DEBUGstart;
          self.logger.info("RDF match time: " + Math.floor(self.store.DEBUGtime));
          self.logger.info("HTTP call time: " +  Math.floor(self.nodes[0]._options.fragmentsClient._httpClient.DEBUGtime));
          self.logger.info("HTTP calls: " +  self.nodes[0]._options.fragmentsClient._httpClient.DEBUGcalls);
