...
//    votes = self.nodes;
//    votes = _.filter(votes, function (node) {
//      return !node.ended() && !(node.activeStream.isHungry && node.activeStream.isHungry());
//    });
    ...
    var suppliers = _.flatten(_.map(boundNodes, function (node) {
      if (node.activeStream.bindVar) {
        return _.pluck(_.without(self.clusters[node.activeStream.bindVar].supplyPath([node]), node), 'pattern');
      }
    }));
...
//      });
//    });
    console.time("POST-READ ADD");
    self.store.addTriples(add, function () {
      console.timeEnd("POST-READ ADD");
      // TODO: timing
...
//      });
    ...
    console.time("POST-READ UPDATES");
    var delayedCallback = _.after(_.size(self.clusters), function () {
      console.timeEnd("POST-READ UPDATES");
      ...
      console.timeEnd("POST-READ SWITCH");

      // TODO: DEBUG let's see how many results we have
      console.time("POST-READ QUERY");
      self.store.matchBindings(_.pluck(self.nodes, 'pattern'), function (results) {
        console.timeEnd("POST-READ QUERY");
        console.time("POST-READ RESULTS");
...
//          }
        ...
        console.timeEnd("POST-READ RESULTS");
        console.timeEnd("POST-READ");

        if (_.size(self.results) >= 10) {
          self.logger.info("TOTAL: " + (new Date() - self.DEBUGstart));
          process.exit(0);
        }
