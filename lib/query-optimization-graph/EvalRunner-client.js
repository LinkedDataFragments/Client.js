...
fs.appendFileSync('eval.log', query.replace(/(\r\n|\n|\r)/gm," ") + '\n');
...
try {
  ...
  sparqlIterator.once('data', function (data) {
  ...
  sparqlIterator.on('end', function () {
    ...
    fs.appendFileSync('eval.log', '!' + [DEBUGfirstTime, DEBUGfirstHttp, DEBUGtime, DEBUGhttp].join(';') + '\n');
  });
}
