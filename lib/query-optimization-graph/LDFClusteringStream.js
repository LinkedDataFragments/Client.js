...
LDFClusteringStream.calls = function (nodes, clusters, store, logger, options) {
  ...
  _.each(incompletes, function (node) {
    logger.info("CALLING " + rdf.toQuickString(node.pattern));
    node.stream.readCall(function (buffer) {
      store.addTriples(buffer, function () {
        // callback
      });
      logger.info("CALLED (" + buffer.length + ") " + rdf.toQuickString(node.pattern));
  ...
  _.each(clusters, function (cluster) {
    ...
    _.each (cluster.nodes, function (node) {
      if (node.stream.estimateRemainingCalls() > bindings.length) {
        logger.info("Updating " + rdf.toQuickString(node.pattern) + " to bind stream on var " + cluster.key);
        node.stream = new BindingStream(node.pattern, bindings, options);
        node.bindVar = cluster.key;
      }
    });
  });
