...
FullDownloadStream.prototype.readCall = function (callback, logger_DEBUG) {
  if (this.ended()) {
  ...
  var val;
  if (logger_DEBUG)
    logger_DEBUG.info("READABLE LISTENERS BEFORE ADD: " + iterator.listeners("readable").length);
  iterator.on('data', addTriple);
  if (logger_DEBUG)
    logger_DEBUG.info("READABLE LISTENERS AFTER ADD: " + iterator.listeners("readable").length);
  iterator.on('end', end);
...
BindingStream.prototype.feed = function (added, removed, moreDataComing) {
  console.error("FEED: " + rdf.toQuickString(this._pattern) + " added: " + (added ? added.length : added) + ", removed: " + (removed ? removed.length : removed) + ", coming: " + moreDataComing);
  ...
  added = _.filter(added, function (binding) { return !ClusteringUtil.containsObject(storedBindings, binding) && !ClusteringUtil.containsObject(self._bindings, binding); });
...
LDFClusteringStream.updateStreams = function (nodes, clusters, store, logger, options) {
  ...
  _.each(newlyEnded, function (node) {
    ...
    _.each(varPos, function (vp) {
      ...
      // TODO: this doesn't update the values stored in the bound stream
      // update remaining node triples with bounds
      _.each(_.filter(cluster.nodes, function (node) { return !node.stream.ended(); }), function (node) {
        ...
        if (node.stream.feed && node.bindVar && groupedTriples.removed) {
          ...
          node.stream.feed([], bindings, true);
        }
      });
    });
  });
  ...
  _.each(clusters, function (cluster) {
    ...
    // TODO: not really needed if no new data has arrived?
    ...
    // TODO: union doesnt work on objects, check results here
    cluster.bindings = _.union(cluster.bindings, groupedBindings.added);
  });
