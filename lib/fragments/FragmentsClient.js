...
FragmentsClient.prototype.getFragmentByPattern = function (pattern) {
  ...
  this._startFragment.getProperty('controls', function (controls) {
    // Replace all variables in the pattern by `null`
    var subject   = rdf.isVariable(pattern.subject)   ? null: pattern.subject;
    var predicate = rdf.isVariable(pattern.predicate) ? null: pattern.predicate;
    var object    = rdf.isVariable(pattern.object)    ? null: pattern.object;
    ...
    fragment.loadFragmentPage(controls.getFragmentUrl(pattern));
  }, this);
