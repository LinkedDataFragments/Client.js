...
function StreamIterator(stream) {
  ...
  // If a readable listener is attached, wait for the stream to become readable
  this._onReadable = this._push.bind(this);
  this.on('newListener', function waitForReadable(event, listener) {
    if (event === 'readable') {
      stream.once('readable', this._onReadable);
...
StreamIterator.prototype._read = function () {
  ...
  this._stream.removeListener('readable', this._onReadable);
  this._stream.once('readable', this._onReadable);
};
