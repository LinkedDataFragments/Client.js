...
Iterator.prototype._fillBuffer = function () {
  ...
  var buffer = this._buffer, wasEmpty = buffer.length === 0, prevBufferLength = -1;
  while ((prevBufferLength !== buffer.length) && (buffer.length < this._bufferSize)) {
    prevBufferLength = buffer.length;
    try { this._read(this._pushBound); }
    catch (readError) { this._error(readError); }
  }
  // Signal if the buffer turned from empty to readable
...
Iterator.prototype.setProperty = function (propertyName, value) {
  ...
  this.emit('propertySet', propertyName, value);
...
TransformIterator.prototype.setSource = function (source) {
  ...
  if (EventEmitter.listenerCount(this, 'readable') !== 0)
    source.on('readable', this._pushBound);
  else
  ...
  if (source.setProperty) {
    ...
    // ToDo: listen for specific properties only
    source.on('propertySet', function (propertyName, value) {
      self.emit('propertySet', propertyName, value);
      source.on(propertyName + 'Set', self.emit.bind(self, propertyName + 'Set'));
    });
  }
};
...
TransformIterator.inherits(CloneableIterator);
...
// Reads an item from the source
CloneableIterator.prototype._read = function (push) {
  var item = this._source.read();
  item && push(item);
};

// Fills the buffer using `_read` to speed up subsequent `read` calls
CloneableIterator.prototype._fillBuffer = function () {
  // Keep filling as long as the length changes and is less than the maximum
  var buffer = this._buffer, prevBufferLength = -1;
  while ((prevBufferLength !== buffer.length) &&
         (buffer.length < this._readPosition + this._bufferSize)) {
    prevBufferLength = buffer.length;
    try { this._read(this._pushBound); }
    catch (readError) { this._error(readError); }
  }
