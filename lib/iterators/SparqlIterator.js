...
function SparqlIterator(source, query, options) {
  ...
  try {
    if (typeof(query) === 'string') {
      ...
      for (var i = 0, l = parsedQuery.filters && parsedQuery.filters.length; i < l; i++) {
        var filter = SparqlExpressionEvaluator(parsedQuery.filters[i]);
        graphIterator = new FilterIterator(graphIterator, filter, options);
      }
      // Create iterators for each order
      for (i = parsedQuery.order && (parsedQuery.order.length - 1); i >= 0; i--) {
        var order = SparqlExpressionEvaluator(parsedQuery.order[i].expression),
            ascending = !parsedQuery.order[i].descending;
      ...
      var queryIterator, QueryConstructor = queryConstructors[parsedQuery.type];
      if (!QueryConstructor)
        throw new Error('No iterator available for query type: ' + parsedQuery.type);
      queryIterator = new QueryConstructor(graphIterator, parsedQuery, options);
      // Create iterators for modifiers
      if (parsedQuery.modifier === 'DISTINCT')
        queryIterator = new DistinctIterator(queryIterator, options);
      queryIterator.parsedQuery = parsedQuery;
