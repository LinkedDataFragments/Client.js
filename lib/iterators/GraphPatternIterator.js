...
GraphPatternIterator.prototype._read = function () {
  ...
  if (!pipeline) {
    ...
    } while (bindings && pipelines.length < this._maxBufferSize);
  ...
  return patternBindings && this._push(patternBindings);
};
...
GraphPatternIterator.prototype._addPipeline = function (bindings) {
  ...
  pipeline.on('readable', function () { self._fillBuffer(); });
  pipeline.on('end',      function () { self._fillBuffer(); });
...
GraphPatternIterator.prototype._flush = function () {
