...
function LinkedDataFragmentsClient(datasource) {
  ...
  this._htmlFragmentParser = new LinkedDataFragmentHtmlParser();
  this._startFragment = performRequest(datasource).then(function (startResource) {
    return self._htmlFragmentParser.parse(startResource.body, startResource.url);
  });
}
...
LinkedDataFragmentsClient.prototype = {
  ...
  _getFragment: function (triplePattern) {
    ...
    .then(function (fragment) {
      return self._htmlFragmentParser.parse(fragment.body, fragment.url);
    },
  ...
  function execute() {
    ...
    request(options, function (error, response, body) {
      ...
      if (error)
        ...
        deferred.resolve({ url: options.url || options, body: body });
