...
LinkedDataFragmentsClient.prototype = {
  ...
  // by following hypermedia controls in the previous fragment.
  // This avoids a client/server contract for specific URL patterns.
  _getFragment: function (previousFragment, triplePattern) {
    ...
    return q.resolve().then(function () {
      return self._fetcher.request(previousFragment.getUrlToFragment(tripleTemplate), method);
    })
    ...
    // extract the fragment data from a successful response
  ...
  _getSubPatternOptions: function (subPattern) {
    ...
    return this._getBestFragments(subPattern).then(function (bestFragments) {
      ...
      var fragment = bestFragments[0],
          // each triple in this fragment leads to possible bindings for the pattern
          fragmentOptions = q.all(fragment.triples.map(function (boundTriple) {
            // fill out the bindings for this specific triple
            var bindings;
            try { bindings = addBindings({}, fragment.triplePattern, boundTriple); }
            // if no consistent binding exists, signal a dead end
            catch (error) { return false; }
            // bind the entire pattern using the bindings from this triple
            var boundPattern = applyBindings(bindings, subPattern);
            boundPattern.fragment = fragment;
            // recursively find options for the bound pattern
  ...
  // Gets a promise for the possible bindings of the given basic graph pattern
  getBindings: function (pattern) {
    // Initialize the entry fragment if necessary
    var self = this;
    if (!this._entryFragment) {
      var fragmentDocument = this._fetcher.repeat(this._datasource);
      this._entryFragment = fragmentDocument.then(function (entryResource) {
    ...
    // When the entry fragment is ready, get the options for the pattern
    return this._entryFragment.then(function (entryFragment) {
      pattern.fragment = entryFragment;
      return self._getPatternOptions(pattern);
    })
    // Return the top bindings object (no bindings, only options)
    .then(function (options) { return { bindings: {}, options: options }; });
  },
...
function isVariable(entity) {
  return (/^urn:var#|^_:/i).test(entity);
}

// Gets all variables and blank nodes in the triple's components
