...
function LinkedDataFragmentsClient(config) {
  ...
  this._startFragment = performRequest(config.datasource).then(function (startResource) {
...
function performRequest(url) {
  ...
  function execute() {
    ...
    request({ url: url, headers: headers, timeout: 5000 }, function (error, response, body) {
      performRequest.pending--;
      if (error)
