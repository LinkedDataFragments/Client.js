...
LinkedDataFragmentsClient.prototype = {
  ...
  _getSubPatternOptions: function (subPattern) {
    ...
    return this._getBestFragments(subPattern).then(function (bestFragments) {
      // if one fragment has no result, the entire subpattern has no consistent bindings
      if (bestFragments.minCount === 0)
        return [];

      // if all fragments have exactly one match, there is one result
      ...
      // some triple patterns have multiple matches; follow the best
