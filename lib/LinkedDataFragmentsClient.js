...
LinkedDataFragmentsClient.prototype = {
  ...
  _getSubPatternOptions: function (subPattern) {
    ...
    return fragments.then(function (fragments) {
      ...
      // if one of the subpattern triples has no result, no consistent result exists; emit null
      ...
      if (_.max(tripleCounts) === 1) {
        ...
        return _.isEmpty(bindings) ? [] : [{ bindings: bindings }];
      }
      ...
      var bestFragment = _.min(fragments, function (f) { return f.matchCount; }),
          ...
          subOptions = q.all(bestFragment.triples.map(function (boundTriple) {
            ...
            .then(function (patternOptions) {
              if (patternOptions !== null) {
                var boundOptions = { bindings: bindings };
                // add all options if there is more than one
                if (patternOptions.length > 1)
                  boundOptions.options = patternOptions;
                // directly add the option if there is only one
                else if (patternOptions.length === 1)
                  boundOptions.options = patternOptions[0].options;
                return boundOptions;
              }
            });
          }));
      // remove inconsistent bindings (null)
  ...
  _getPatternOptions: function (pattern) {
    ...
    return optionsPerSubPattern.then(function (optionsPerSubPattern) {
      // if one of the subpatterns has no solutions, there is no global solution; emit null
      if (_.contains(optionsPerSubPattern, null))
        return null;

      // only mention those that offer at least one option
      optionsPerSubPattern = _.reject(optionsPerSubPattern, _.isEmpty);

      // combine the options
      while (optionsPerSubPattern.length > 1) {
        optionsPerSubPattern = _.sortBy(optionsPerSubPattern, 'length');
        var parent  = optionsPerSubPattern.shift(),
            options = optionsPerSubPattern.shift();
        parent.forEach(function addOptions(child) {
        ...
        optionsPerSubPattern.push(parent);
      }
      return optionsPerSubPattern[0] || [];
    });
  },
  ...
  getQueryResults: function (sparql) {
    ...
    // options can be null if no consistent binding exists
