...
function SparqlIterator(source, query, options) {
  ...
  try {
    ...
    options.graph = ((query.from || {}).default || [null])[0];
    ...
    if (!options.graph && options.graphsNamed.length > 0) options.graph = '__empty__';
    if (!QueryConstructor)
...
function SparqlGroupIterator(source, group, options) {
  ...
  case 'graph':
    options = _.create(options, { graph: group.name });
