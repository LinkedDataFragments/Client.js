...
function TurtleFragmentIterator(source, fragmentUrl) {
  ...
  this._parser.parse(function (error, triple) {
    triple && self._processTriple(triple) ||
...
TurtleFragmentIterator.prototype._processTriple = function (triple) {
  ...
  if (triple.subject !== this._fragmentUrl && triple.predicate.indexOf(rdf.HYDRA) !== 0)
    ...
    this._pushMetadata(triple);
};

// Adds a triple to the metadata stream
TurtleFragmentIterator.prototype._pushMetadata = function (triple) {
...
TurtleFragmentIterator.prototype._flush = function () {
  ...
  // TODO: fix the <fragment-end> hack below.
  // There seems to be an issue when combining cloned iterators and MultiTransformIterator,
  // in which one of the iterators stops reading sometimes.
  // Adding a dummy final element fixes this in many cases.
  // This dummy element is to be ignored by TriplePatternIterator.
  this._push({ subject: '<fragment-end>', predicate: '', object: '' });
