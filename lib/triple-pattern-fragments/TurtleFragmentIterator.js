...
function TurtleFragmentIterator(source, fragmentUrl) {
  ...
  var self = this;
  this._turtleParser = new N3.Parser({ documentURI: fragmentUrl });
  this._turtleParser.parse(function (error, triple) {
...
TurtleFragmentIterator.prototype._transform = function (chunk, done) {
  this._turtleParser.addChunk(chunk), done();
};
...
TurtleFragmentIterator.prototype._processTriple = function (triple) {
  // Note: this separation between data and metadata/controls is an approximation;
  // for a proper separation, we need a triple serialization format with graph support.
  if (triple.predicate !== rdf.VOID_TRIPLES && triple.predicate.indexOf(rdf.HYDRA) !== 0 && triple.predicate.indexOf(rdf.DS) !== 0)
    this._push(triple);
  else
