...
TurtleFragmentParser.prototype._fields = ['subject', 'predicate', 'object'];
...
TurtleFragmentParser.prototype.parseControls = function (controlData) {
  ...
  if (searchForms) {
    ...
    assert(mappings.length === this._fields.length,
        'Expected ' + this._fields.length + ' hydra:mappings for ' + searchForm);
    ...
    controls.getFragmentUrl = function (triplePattern) {
      ...
      self._fields.forEach(function (field) {
        variables[mappings[rdf.RDF + field]] = triplePattern[field];
      });
