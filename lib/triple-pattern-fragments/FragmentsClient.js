...
function FragmentsClient(startFragment, options) {
  ...
  if (startFragment) {
    ...
    startFragment.on('error', function (error) { startFragment.error = error; });
    startFragment.getProperty('controls', function () { startFragment.error = null; });
  }
}
...
FragmentsClient.prototype.getFragmentByPattern = function (pattern) {
  ...
  //Check wether the pattern has a parent with bloom filter
  var filter;
  if (!rdf.isVariable(pattern.subject) && !rdf.isVariable(pattern.predicate) && !rdf.isVariable(pattern.object)) {
    filter = _.find(this._filters, function (bloom, parentPattern) {
      return isBoundPatternOf(pattern, parentPattern);
    });
  }
  ...
  if (startFragment.error !== null) {
    ...
    startFragment.on('error', startFragmentError);
  }
  ...
  startFragment.getProperty('controls', function (controls) {
    ...
    //if (filter && filter.test(fragmentUrl))
    if (!filter || filter.test(JSON.stringify(pattern)))
