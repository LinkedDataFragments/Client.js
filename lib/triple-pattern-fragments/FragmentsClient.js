...
function FragmentsClient(startFragment, options) {
  ...
  if (typeof startFragment !== 'string')
    ...
    (this._startFragment = new Fragment(this)).loadFromUrl(startFragment, options.headers);
  }
}

// Returns the Triple Pattern Fragment for the given triple pattern
FragmentsClient.prototype.getFragmentByPattern = function (pattern, headers) {
  ...
  this._startFragment.getProperty('controls', function (controls) {
    ...
    fragment.loadFromUrl(controls.getFragmentUrl(pattern), headers);
  });
...
Fragment.prototype.loadFromUrl = function (pageUrl, headers) {
  // Fetch the page of the fragment
  var self = this, fragmentsClient = this._fragmentsClient, fragmentPage;
  _.defaults(headers || {}, {'user-agent': 'Triple Pattern Fragments Client'});

  if (fragmentsClient._startFragmentUrl && !headers.referer) headers.referer = fragmentsClient._startFragmentUrl;
