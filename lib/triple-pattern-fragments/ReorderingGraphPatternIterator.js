...
ReorderingGraphPatternIterator.prototype._createTransformer = function (bindings, options) {
  ...
  // Select the smallest connected subpattern with the least number of variables in the resulting pattern
  var subPatterns = _.sortBy(rdf.findConnectedPatterns(boundPattern), function (patterns) {
    var sortVal = _.reduce(patterns, function (union, pattern) { return _.union(union, rdf.getVariables(pattern)); }, []).length;
    return -(boundPattern.length*sortVal + patterns.length); // reverse sort so we can get the lowest value with a pop
  });
  var subPattern = subPatterns.pop(), pipeline;
