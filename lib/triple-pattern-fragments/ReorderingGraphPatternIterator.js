...
ReorderingGraphPatternIterator.prototype._createTransformer = function (bindings, options) {
  ...
  var subPatterns = _.sortBy(rdf.findConnectedPatterns(boundPattern), function (patterns) {
    return -_.reduce(patterns, function (union, pattern) { return _.union(union, rdf.getVariables(pattern)); }, []).length;
  });
  subPatterns = _.sortBy(subPatterns, function (patterns) { return -patterns.length;} );
