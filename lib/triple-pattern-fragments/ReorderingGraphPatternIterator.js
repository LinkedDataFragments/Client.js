...
var Iterator = require('../iterators/Iterator'),
    ...
    _ = require('lodash'),
    ExecutionLogger = require('../util/ExecutionLogger'),
...
function ReorderingGraphPatternIterator(parent, pattern, options) {
  ...
  this._logger = new ExecutionLogger(options.logger || Logger('ReorderingGraphPatternIterator'));
}
...
ReorderingGraphPatternIterator.prototype._createTransformer = function (bindings, options) {
	var self = this;
	//self._logger.info('Current bindings: ', bindings);
  ...
  subPattern.forEach(function (triplePattern, index) {
    ...
    fragment.getProperty('metadata', function (metadata) {
      // Add logging info
      self._logger.log(bindings, triplePattern, metadata.totalTriples);
