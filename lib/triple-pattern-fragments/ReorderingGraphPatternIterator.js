...
function ReorderingGraphPatternIterator(parent, pattern, options) {
  ...
  // TODO: not doing this to have filter code
...
ReorderingGraphPatternIterator.prototype._createTransformer = function (bindings, options) {
  // TODO: check if binding is accepted by filters
  ...
  var bestIndex = 0, minMatches = Infinity, bestFilter = null, patternsChecked = 0;
      ...
      if (filters.length > 0) {
        ...
        filters.forEach(function (filter) {
          filter.totalTriples(function (count) {
            // TODO: hardcoded pagesize (1 for testing purposes)
            if (count * 1 < minMatches)
  ...
  function createPipeline(triplePattern, filter) {
    ...
    // TODO: only works for unbound triples ...
    var startIterator = Iterator.single(bindings),
        pipeline = new TriplePatternIterator(filter ? filter.copy(startIterator) : startIterator, triplePattern, options);
