...
ReorderingGraphPatternIterator.prototype._createTransformer = function (bindings, options) {
  ...
  subPattern.forEach(function (triplePattern, index) {
    ...
    fragment.getProperty('metadata', function (metadata) {
      ...
      //Inititalize bloom filter
      if (metadata.filter) {
        var m = Math.ceil((-metadata.totalTriples * Math.log(0.01)) / (Math.LN2 * Math.LN2)),
            k = Math.round((m / metadata.totalTriples) * Math.LN2);

        var bloom = new BloomFilter(m, k);
        bloom.buckets = metadata.filter;

        this._client._filters[triplePattern] = bloom;
      }
