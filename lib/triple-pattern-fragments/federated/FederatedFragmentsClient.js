...
FederatedFragmentsClient.prototype.getStartFragment = function () {
  // TODO: somehow combine the multiple fragment metadata properties
  var fragments = _.map(this._clients, function (client) { return client.getStartFragment(); });
...
function CompoundFragment(fragments, options) {
  ...
  _.each(pendingFragments, function (fragment, index) {
    ...
    var freeTextDone = _.once(function (fragmentFreeText) {
      // Sum the metadata if it exists
      if (fragmentFreeText)
        combinedFreeText &= fragmentFreeText;
      // If no metadata is pending anymore, we can emit it
      if (--pendingFreeTextCount === 0)
        self.setProperty('freeText', combinedFreeText);
    });
    fragment.getProperty('freeText', freeTextDone);
