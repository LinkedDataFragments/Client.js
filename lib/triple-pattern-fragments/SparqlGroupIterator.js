...
function SparqlGroupIterator(source, group, options) {
  ...
  if (_.isArray(group)) {
    ...
    // TODO: naming conventions and substring stuff

    options.fragmentsClient.getStartFragment().getProperty('metadata', function (metadata) {
      if (metadata && metadata.substring) {
        var grouped = _.groupBy(group, function (subGroup) {
          if (subGroup.type === 'filter' && subGroup.expression.operator === 'regex') {
            try {
              ...
              if (!options.filters) {
                // make sure this doesn't spread out of scope
                options = _.create(options, {filters: {}});
                options.filters = {};
              }
        ...
        // TODO: also appending regex filters here atm to make sure they always get applied, should somehow detect which filters are required here
        group = (grouped.default || []).concat(grouped.substring || []);
      }

      var sparqlIt = group.reduce(function (source, subGroup) {
        return new SparqlGroupIterator(source, subGroup, options);
      }, source);

      pipeline.setSource(sparqlIt);
    });
