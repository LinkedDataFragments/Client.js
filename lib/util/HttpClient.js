...
HttpClient.prototype.request = function (url, method, headers, options) {
  ...
  function performRequest() {
    self._logger.info('Requesting', url, 'with headers', JSON.stringify(headers));
    ...
    request.on('response', function (response) {
      ...
      if (headers.link) {
        ...
        //console.log(parsed);
        if (parsed.timegate && !headers["memento-datetime"] && self._acceptDatetime) {
          var timegateResponse = self.request(parsed.timegate.url, method, _.extend({'Accept-Datetime': self._acceptDatetime.toUTCString()}, headers), options);
