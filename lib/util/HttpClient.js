...
function HttpClient(options) {
  ...
  if (options.datetime) this._acceptDatetime = options.datetime;
}
...
HttpClient.prototype.request = function (url, method, headers, options) {
  ...
  function performRequest() {
    ...
    if (self._acceptDatetime) acceptHeaders['Accept-Datetime'] = self._acceptDatetime.toUTCString();
    var requestOptions = {
      url: url,
      method: method || 'GET',
    ...
    self._logger.info('Requesting', url, 'with headers', JSON.stringify(requestOptions.headers));
    ...
    request.on('response', function (response) {
      ...
      // Follow link to timegate if datetime was given
      if (headers.link && self._acceptDatetime) {
        var parsed = parseLink(headers.link);
        //console.log(parsed);
        if (parsed.timegate && !headers["memento-datetime"]) {
          var timegateResponse = self.request(parsed.timegate.url, method, headers, options);
          responseIterator.setSource(timegateResponse);
