...
evaluators = {
  ...
  operation: function (expression) {
    ...
    var minArgs = operator.minimumArgumentsLength || operator.length;
    if (expression.args.length < minArgs || expression.args.length > operator.length)
      throw new InvalidArgumentsNumberError(operatorName, expression.args.length, minArgs, operator.length);
