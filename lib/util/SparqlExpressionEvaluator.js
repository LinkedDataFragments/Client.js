...
evaluators = {
  ...
  operation: function (expression) {
    ...
    var minArgs = operator.length - (operator.optionalArguments || 0);
    var maxArgs = operator.length;
    if (expression.args.length < minArgs || expression.args.length > maxArgs)
      throw new InvalidArgumentsNumberError(operatorName, expression.args.length, minArgs, maxArgs);
...
operators = {
  ...
  'regex': function (subject, pattern, flag) {
    if (isLiteral(subject))
      subject = literalValue(subject);
    return new RegExp(literalValue(pattern), flag).test(subject);
  },
  'str': function (a) {
...
// Tag all operators with optional arguments
operators.regex.optionalArguments = 1;
