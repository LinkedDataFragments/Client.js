...
/**
 * Disables all loggers at or below the specified level.
...
Logger.disable = function (level) {
  ...
  else if ((level = _.indexOf(LOG_LEVELS, level.toUpperCase())) >= 0)
    // Disable loggers below a specific level
    while (level < LOG_LEVELS.length)
      Logger.prototype[LOG_LEVELS[level++].toLowerCase()] = _.noop;
};
